...
parser.add_argument('data', type=str,
                    help='file containing dataset paths')
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet50',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet50)')
parser.add_argument('-j', '--workers', default=80, type=int, metavar='N',
...
parser.add_argument('--icutmix', action='store_true',
                    help='apply input cut mix augmentation')
parser.add_argument('--alpha', default=1.0, type=float,
                    help='parameter alpha used in beta distribution')
...
parser.add_argument('--save_folder', default='./output/debug',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        model = moco.builder.MoCo_ViT(
            ...
            icutmix=args.icutmix, alpha=args.alpha)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            filename = os.path.join(args.save_folder, 'checkpoint_%04d.pth.tar' % epoch)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    while True:
        try:
            print(f'=======> attempt model saving at: {filename}')
            torch.save(state, filename)
            break
        except:
            print(f'=======> model saving failed at: {filename}')
